...
- **구성 검증**: 현재 구조를 순수 MSA가 아니라 폴리레포 서비스 + 중앙 PostgreSQL/Flyway 스키마를 쓰는 `distributed modular monolith`로 설명합니다.
- **통신 흐름**: Flutter 클라이언트는 gateway만 호출하고, gateway가 platform/learning/ai/sandbox/community 서비스로 라우팅합니다.

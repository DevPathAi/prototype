...
- **데모**: 진단, 학습경로 생성, 샌드박스 코드리뷰, AI 검색/추천을 각각 시뮬레이터로 체험합니다.
...
- **진단 시뮬레이터**: 학습자 페르소나를 선택하면 역량 점수, 권장 트랙, 우선 보강 영역이 바뀝니다.
- **학습경로 생성 시뮬레이터**: 목표 트랙, 주간 학습 시간, 약점 영역을 조정하면 생성 경로가 바뀝니다.
- **샌드박스 코드리뷰 시뮬레이터**: 코드 샘플을 선택하고 AI 리뷰 실행 버튼을 눌러 심각도, 리뷰 요약, 수정 제안을 확인합니다.
- **AI 검색/추천 시뮬레이터**: 검색 의도와 pgvector 포함 여부를 바꾸며 키워드/임베딩 혼합 추천 결과를 비교합니다.
